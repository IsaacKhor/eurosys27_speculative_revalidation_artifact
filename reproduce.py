#!/usr/bin/env python3
"""Reproduce the paper's empirical figures from prepared public traces.

Run ``download_traces.py`` first to prepare the public inputs. Source files,
generated data, build products, tool caches, logs, models, results, and figures
all live below this artifact directory.
"""
...
class Pipeline:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.layout = Layout(HERE / "smoke" if args.smoke else HERE)
    ...
    def prepare(self) -> None:
        ...
        if not self.args.dry_run:
            ...
            manifest.write_text(json.dumps({
                ...
                "key_sample_ratio": self.args.key_sample_ratio or (8 if self.args.smoke else 1),
                "parallel": self.args.parallel,
                "input_scope": SMOKE_ROWS if self.args.smoke else "full trace",
                "preprocessing": "all selected rows",
            }, indent=2) + "\n", encoding="utf-8")

    def bootstrap_python(self) -> None:
        project_files = (HERE / "uv.lock", HERE / "pyproject.toml")
        if self.completed("python", (self.python,), project_files):
            self.announce("reuse local Python environment")
            return
        self.run(
            "create local locked Python environment",
            ("uv", "sync", "--frozen", "--no-install-project", "--project", HERE),
            cwd=self.layout.root,
        )
        self.mark_completed("python", (self.python,), project_files)
    ...
    def build(self) -> tuple[Path, Path]:
        ...
        build_inputs = tuple(sorted(
            path for path in (HERE / "sim").iterdir()
            if path.suffix in (".cpp", ".h", ".hpp")
        )) + (HERE / "xmake.lua",)
        build_config = {
            "smoke": self.args.smoke,
            "smoke_rows": SMOKE_ROWS if self.args.smoke else None,
        }
        ...
        self.announce("stage simulator sources", (HERE / "sim", stage / "sim"))
        if not self.args.dry_run:
            self._assert_output(stage)
            if stage.exists():
                shutil.rmtree(stage)
            stage.mkdir(parents=True)
            shutil.copy2(HERE / "xmake.lua", stage / "xmake.lua")
            shutil.copytree(HERE / "sim", stage / "sim")
            if self.args.smoke:
    ...
    def train_models(self, specs: Sequence[TraceSpec]) -> dict[str, Path]:
        ...
        for dataset in ("cf", "fb", "wm"):
            ...
            training_inputs = tuple(self.expiry_path(spec) for spec in selected) + (
                HERE / "sim" / "train_ml.py", HERE / "seeded_train.py",
            )
            sample_frac = (
                "1" if self.args.smoke
    ...
    def run_experiments(
        ...
        cache_types = ("lru",) if self.args.smoke else FULL_CACHE_TYPES
        ...
        oracle_thresholds = ("1",) if self.args.smoke else FULL_ORACLE_THRESHOLDS
        ...
        ml_thresholds = ("1", "0.8") if self.args.smoke else FULL_ML_THRESHOLDS
        ...
        if time_specs:
            ...
            thresholds = ("1", "0.8") if self.args.smoke else FULL_ML_THRESHOLDS
    ...
    def selected_specs(self) -> tuple[TraceSpec, ...]:
        return tuple(
            ...
            if not self.args.smoke or spec.sim_name in SMOKE_NAMES
        )

    def write_manifest(self) -> None:
        self.announce("verify empirical figures and write output manifest")
        if self.args.dry_run:
            return
        manifest_path = self.layout.output / "manifest.json"
        self._assert_output(manifest_path)
        files = [self.layout.figures / f"{name}.png" for name in FIGURE_NAMES]
        missing = [path for path in files if not path.is_file() or path.stat().st_size == 0]
        if missing:
            shown = "\n".join(f"  - {path}" for path in missing)
            raise RuntimeError(f"plotting did not produce every empirical figure:\n{shown}")
        manifest = {
    ...
    def execute(self) -> None:
        ...
        ksr = self.args.key_sample_ratio or (8 if self.args.smoke else 1)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--plots-only", action="store_true",
                        help="regenerate plots from artifact-local results and models")
